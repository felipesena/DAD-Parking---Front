    ...
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on }">
          <v-btn color="primary" dark class="mb-2" v-on="on">Criar Vinculo</v-btn>
        </template>
        <v-card>
          <v-card-title>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      deleteVinculo: types.DELETE_VINCULO
    }),
    initialize() {
      this.getAllVinculos();
    },
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        await this.updateVinculo(updatedVinculo);
      } else {
        await this.createVinculo({tipoVeiculo: this.editedItem.tipoVeiculo});
      }
      this.close();
    }
  }
